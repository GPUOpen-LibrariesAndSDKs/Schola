...
- ``FabricaGeneratedRewardForAgent`` — add shaped terms to ``FAgentState::Info`` (passed as ``RewardComponents``) using the prefix from ``UFabricaRewardInfo::GetComponentPrefix()`` (default ``fabrica_r:``). ``AFabricaEnvironment::Step`` sets ``FAgentState::Reward`` to the sum of those entries.

``OnUserStep`` fills one ``FFabricaAgentState``. ``AFabricaEnvironment::Step`` maps it under ``GetFabricaSingleAgentId()``, converts to ``FAgentState``, copies ``TaskSuccessMetric`` into ``Info`` under ``UFabricaRewardInfo::GetTaskSuccessInfoPrefix()`` (default ``fabrica_ts``), passes ``Info`` to ``FabricaGeneratedRewardForAgent``, then sets ``Reward`` from the sum of ``fabrica_r:`` entries. Python training logs read the task-success key on episode end.
